...
public class BusinessMemberManageController {
	...
	public String viewMember(HttpSession session, Model model) {
		...
		BusinessMember member = businessService.findById(presentBusinessMember.getId());
		if (member == null) {
			throw new RuntimeException("Member not found with id: " + presentBusinessMember.getId()); // 防止 null 返回
		}
