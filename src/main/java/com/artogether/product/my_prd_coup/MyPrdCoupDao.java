...
public interface MyPrdCoupDao {
	...
	List<MyPrdCoup> findCouponsExpiringSoonForMember(Integer memberId, LocalDateTime currentDate, Integer days);// 查詢即將到期的優惠券

	boolean receiveCoupon(MyPrdCoup.MyPrdCoupId myPrdCoupId, LocalDateTime receiveDate);// 新增領取優惠券功能

	boolean useCoupon(MyPrdCoup.MyPrdCoupId myPrdCoupId, LocalDateTime useDate);// 更新優惠券的使用狀態
